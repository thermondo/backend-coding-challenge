...
class UserInfoEndPoint(EndPoint):
    ...
    async def generate_activation_code(
        ...
        name: Annotated[
            str,
            Path(
                description='The name of the user to generate activation code for.', max_length=16
            ),
        ],
    ) -> ResponseModel:
        """
    ...
    async def activate_account(
        ...
        activation_token: Annotated[
            str,
            Path(
                description='The generated activation code to activate user account.', max_length=32
            ),
        ],
    ) -> ResponseModel:
        """
        ...
        Args:
            activation_token (str): The activation token received by the user.
        ...
        """
        try:
            activation_data = self._user_interactor.get_activation_data(activation_token)
    ...
    async def change_password(self, user_data: ChangePasswordAPIRequest) -> ResponseModel:
        ...
        try:
            ...
            if user_info_model.activation_code != user_data.activation_code:
                return ResponseModel(
                    ChangePasswordResponse(
                        status_code=404,
                        user=self._user_interactor.to_user_response(user_info_model),
    ...
    def router(self) -> APIRouter:
        ...
        router.add_api_route('/{activation_token}', self.activate_account, methods=['GET'])
