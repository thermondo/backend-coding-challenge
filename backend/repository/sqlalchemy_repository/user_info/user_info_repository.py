...
class SQLAlchemyUserInfoRepository(UserInfoRepository):
	...
	def select_users(self) -> List[UserInfoModel]:
		...
		Returns:
			List[UserInfoModel]: List of all existing users.
		"""
	...
	def select_user_by_name(self, name: str) -> Optional[UserInfoModel]:
		"""
		Retrieve a user info object using the name as identifier.

		Parameters:
		    name (str): the user name to be retrieved

		Returns:
			Optional[UserInfoModel]: Existing user with the exact same name, otherwise None.
		"""
		select_stmt = select_user_info_as_json().where(user_info_table.columns.name == name)
		result = self._session.execute(select_stmt).first()
		if result:
			return to_user_info(result[0])
		return None
